...
CLIENT_COMMON = err file net vec
CLIENT_MAIN   = client
...
SERVER_COMMON = err file net vec
SERVER_MAIN   = server
...
PARTIAL_O = partial_credit/crypto.o
...
$(ODIR)/client.exe: $(CLIENT_O) $(PARTIAL_O)
	@echo "[LD] $^ --> $@"
	@$(CXX) $^ -o $@ $(LDFLAGS)
$(ODIR)/server.exe: $(SERVER_O) $(PARTIAL_O)
	@echo "[LD] $^ --> $@"
	@$(CXX) $^ -o $@ $(LDFLAGS)
