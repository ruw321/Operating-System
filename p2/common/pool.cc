...
struct thread_pool::Internal {
  ...
  function<bool(int)> handlerStore;
  ...
  std::function<void()> lambda = [&](){
    ...
    while(true){
      ...
      //queue is not empty
        ...
        int currentSocket = sockets.front();
        sockets.pop();
        lock.unlock();
        active = !handlerStore(currentSocket);
        close(currentSocket);
...
thread_pool::thread_pool(int size, function<bool(int)> handler)
    : fields(new Internal(handler)) {
      for(int i = 0; i < size; i++) {
        fields->threads.push_back(std::thread(fields->lambda));
      }
    }
...
void thread_pool::set_shutdown_handler(function<void()> func) {
  fields->shutDownFunction = func;
}

/// Allow a user of the pool to see if the pool has been shut down
bool thread_pool::check_active() { return fields->active; }
...
void thread_pool::await_shutdown() {
  for(unsigned int i = 0; i < fields->threads.size(); i++) {
    fields->threads[i].join();
  }
}
